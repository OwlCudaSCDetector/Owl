...
#define MAX_FLOW_LEN 65536//32768//16384
#define ROUND 1
#define MAX_MSG 1
					 ...
					 unsigned flow_len)
{
	...
	//start actual test
	unsigned rounds = ROUND;
	uint64_t elaplsed_time[ROUND];
...
void test_aes_enc(int size)
{
	...
	for (unsigned i = 1; i <= MAX_MSG;  i *= 2)
		      ...
		      bool              encrypt)
{
	...
	assert(num_flows > 0 && num_flows <= MAX_MSG);
	assert(key_bits == 128);
	assert(ops != NULL);
